...
function(GetGitState _working_dir)
    ...
    if(NOT exit_code EQUAL 0)
        ...
        set(ENV{GIT_DIRTY_STRING} "")
    else()
        if(NOT "${output}" STREQUAL "")
            set(ENV{GIT_IS_DIRTY} "true")
            set(ENV{GIT_DIRTY_STRING} "-dirty")
        else()
            set(ENV{GIT_IS_DIRTY} "false")
            set(ENV{GIT_DIRTY_STRING} "")
        endif()
    ...
    RunGitCommand(describe --tags --always --long ${object})
    if(NOT exit_code EQUAL 0)
    ...
    # Get major, minor and patch from GIT_DESCRIBE
    string(REGEX MATCH "v([0-9])+\\.([0-9]+)\\.([0-9]+)" _ "$ENV{GIT_DESCRIBE}")
    set(ENV{TAG_MAJOR} "${CMAKE_MATCH_1}")
    set(ENV{TAG_MINOR} "${CMAKE_MATCH_2}")
    set(ENV{TAG_PATCH} "${CMAKE_MATCH_3}")
    if(NOT (${CMAKE_MATCH_1} MATCHES "^[0-9]+$" AND "${CMAKE_MATCH_2}" MATCHES "^[0-9]+$" AND "${CMAKE_MATCH_3}" MATCHES "^[0-9]+$"))
        message(FATAL_ERROR "No valid tag found")
    endif()
